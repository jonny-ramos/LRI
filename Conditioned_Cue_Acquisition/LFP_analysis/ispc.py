...
from lfp import load_data, compute_mwt, ispc
...
# plot
def plot_ispc(ispc, t_zero, freqs, label):
    f, ax = plt.subplots(1,1)
    f.set_figheight(4)
    f.set_figwidth(7)

    fs = 2000
    time = np.arange((0-t_zero) / fs, (np.shape(ispc)[1] - t_zero) / fs, 1/fs)
    ...
    # plot contour fill
    cs = ax.contourf(time, freqs, ispc, levels=levels, cmap='jet')
    ...
    #np.save(' '.join([title, frange,'Spectrogram.npy']), ispc, allow_pickle=True)
...
def main():
    ...
    for trial_n in trial_ns:
        ...
        pha = [f_pha[400+b_i:3400] for f_pha in pha]     # cutting off edges (consistent w/ spectrograms)
    ...
    # compute ispc for each sample for each f across all trials
    ispcs = ispc(pha_trials_r1, pha_trials_r2)

    # plot
    plot_ispc(ispcs, t_0, freqs, label)
