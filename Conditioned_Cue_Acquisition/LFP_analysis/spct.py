...
from lfp import load_data, compute_mwt_fwhm, baseline_norm
...
def main():
    ...
    fs = 2000
    b_i = 1500  # 750 ms baseline period right before plot
    t_0 = 500   # t=0 occurs 500 samples into the epoch after removing baseline
                # (the first 1500 samples); -250ms : +1000ms to plot
    ...
    norm_trials = []
    for sig in data:
        raw_pwr = [abs(compute_mwt_fwhm(sig, f))**2 for f in freqs]
