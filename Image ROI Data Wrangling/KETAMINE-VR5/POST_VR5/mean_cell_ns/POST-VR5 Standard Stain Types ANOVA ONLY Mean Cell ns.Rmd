---
title: "POST VR5-KET Image Data ANOVAs"
...
eda_anova_1way <- function(fname)
  ...
  {
  df <- read.csv(fname, header=TRUE, sep=",")
  df$treat_factor <- as.factor(df$treatment)
  
  ### check assumption of normality
  # quantitative assessment
  print(tapply(df$mean_cell_n, df$treat_factor, shapiro.test))
  
  # qualitative assessment
  g <- ggqqplot(df, x="mean_cell_n", facet.by=c("treat_factor"))
  g
  
  ### check assumption of equal variances
  # quantitative assessment
  print(leveneTest(y=df$mean_cell_n, group=df$treat_factor, center='mean'))
  ...
  df.lm <- lm(mean_cell_n ~ treat_factor, data=df)
  df.III.aov <- car::Anova(df.lm, type = 3)
  print(df.III.aov)
...
```{r}
mean_cell_ns <- list.files(pattern='mean_cell')
```
